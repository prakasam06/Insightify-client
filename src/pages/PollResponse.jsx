...
export const PollResponse = () => {
    ...
    useEffect(() => {
        ...
        const message = "hello guyss from client"
    ...
    const sendMessage = (message) => {
        io('http://localhost:3000/chat/pollresp').emit('getResponse', { params, message });
    };

    return (
        <div>
            <h2>Chat for Poll ID: {params}</h2>
            <button onClick={() => sendMessage('Hello, this is a test message!')}>Send Test Message</button>
        </div>
    );
};
